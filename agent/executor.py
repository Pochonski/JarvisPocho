...
def _call_tool(tool: str, parameters: dict, speak: Callable | None) -> str:
    ...
    elif tool == "opencode_runner":
        from actions.opencode_runner import run_with_opencode
        return run_with_opencode(parameters=parameters, player=None) or "Done."
...
class AgentExecutor:
    ...
    def execute(
        ...
        while True:
            ...
            if not steps:
                msg = "I couldn't create a valid plan for this task, jefe."
                if speak: speak(msg)
                return msg
            ...
            for step in steps:
                if cancel_flag and cancel_flag.is_set():
                    if speak: speak("Task cancelled, jefe.")
                ...
                while attempt <= 3:
                    ...
                    try:
                        ...
                        if decision == ErrorDecision.RETRY:
                            ...
                            msg = f"Task aborted, jefe. {recovery.get('reason', '')}"
                            if speak: speak(msg)
                            return msg
                            ...
                            if fix_suggestion and tool != "generated_code":
                                try:
                                    ...
                                    if speak: speak("Trying an alternative approach, jefe.")
                                    res = _call_tool(
                                        fixed_step["tool"],
            ...
            if replan_attempts >= self.MAX_REPLAN_ATTEMPTS:
                msg = f"Task failed after {replan_attempts} replan attempts, jefe."
                if speak: speak(msg)
                return msg

            if speak: speak("Adjusting my approach, jefe.")

            replan_attempts += 1
            plan = replan(goal, completed_steps, failed_step, failed_error)

    def _summarize(self, goal: str, completed_steps: list, speak: Callable | None) -> str:
        fallback = f"All done, jefe. Completed {len(completed_steps)} steps for: {goal[:60]}."
        try:
            import google.generativeai as genai
            genai.configure(api_key=_get_api_key())
            ...
            prompt    = (
                ...
                "Address the user as 'jefe'. Be direct and positive."
            )
            response = model.generate_content(prompt)
