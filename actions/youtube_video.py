...
def _summarize_with_gemini(transcript: str, video_url: str) -> str:
    ...
    model = genai.GenerativeModel(
        ...
        system_instruction=(
            ...
            "Be direct. Address the user as 'jefe'. "
...
def _handle_play(parameters: dict, player) -> str:
    ...
    if not query:
        return "Please tell me what you'd like to watch, jefe."
...
def _handle_summarize(parameters: dict, player, speak) -> str:
    ...
    if not url:
        return "No URL provided, jefe. Summary cancelled."
    if not _is_valid_youtube_url(url):
        return "That doesn't appear to be a valid YouTube URL, jefe."

    video_id = _extract_video_id(url)
    if not video_id:
        return "Could not extract video ID from that URL, jefe."

    if player:
        player.write_log(f"[YouTube] Summarizing: {url}")
    if speak:
        speak("Fetching the transcript now, jefe. One moment.")

    transcript = _get_transcript(video_id)
    if not transcript:
        return "I couldn't retrieve a transcript for that video, jefe."
    ...
    try:
        ...
        return f"Summary generation failed, jefe: {e}"
...
def _handle_get_info(parameters: dict, player, speak) -> str:
    ...
    if not url or not _is_valid_youtube_url(url):
        return "Please provide a valid YouTube URL, jefe."

    video_id = _extract_video_id(url)
    if not video_id:
        return "Could not extract video ID, jefe."
    ...
    if not info:
        return "Could not retrieve video information, jefe."
    ...
    if speak:
        speak(f"Here's the video info, jefe. {result.replace(chr(10), '. ')}")
...
def _handle_trending(parameters: dict, player, speak) -> str:
    ...
    if not trending:
        return f"Could not fetch trending videos for region {region}, jefe."
    ...
    if speak:
        top3   = trending[:3]
        spoken = "Here are the top trending videos, jefe. " + ". ".join(
...
def youtube_video(
    ...
    try:
        ...
        return f"YouTube {action} failed, jefe: {e}"