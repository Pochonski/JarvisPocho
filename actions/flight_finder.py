...
def _format_spoken(
    ...
    if not flights:
        return (
            ...
            f"on {date}, jefe. The page may not have loaded correctly."
        )

    lines = [f"Here are the top flights from {origin} to {destination} on {date}, jefe."]
...
def flight_finder(parameters: dict, player=None, speak=None) -> str:
    ...
    if not origin or not destination:
        return "Please provide both origin and destination, jefe."
    if not date_raw:
        return "Please provide a departure date, jefe."
    ...
    if speak:
        speak(f"Searching flights from {origin} to {destination} on {date}, jefe.")
    ...
    try:
        ...
        if not raw_text:
            return "Could not retrieve flight data, jefe. The page may not have loaded."

        if speak:
            speak("Analysing the results now, jefe.")
        ...
        return f"Flight search failed, jefe: {e}"